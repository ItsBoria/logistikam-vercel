...
- Pending Unit join/access requests are handled in `src/routes/admin.users.tsx` under the default `בקשות גישה` tab. The bottom admin nav exposes `/admin/users` as `משתמשים` to Owner, Work Manager, and admin-level Unit roles and shows a badge from `listUnitAccessRequests`. `UNIT_USER_MANAGER_ROLES` in `src/lib/membership.functions.ts` includes `LOGISTICS_NCO`, so both `מנהל עבודה` and `נגד לוגיסטיקה` can approve/reject users for their active Unit while staying scoped to that Unit.
- Legacy global `ADMIN` / `WORK_MANAGER` accounts from the pre-multi-Unit app are bridged into the `ORIGINAL` Unit in `src/lib/membership.functions.ts`. When such a user has no active Unit membership, `listActiveUnits`, `getMyActiveUnit`, and active-Unit assertions create/restore an active `unit_memberships` row for `ORIGINAL` only (`ADMIN` -> `LOGISTICS_NCO`, `WORK_MANAGER` -> `WORK_MANAGER`). This keeps old original-Unit admins able to load dashboard/orders/products without granting access to other Units.
