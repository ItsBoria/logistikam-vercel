...
function RaspDashboard() {
  ...
  const [issueEditing, setIssueEditing] = useState<any>(null);
  ...
  const upsertIssueFn = useServerFn(upsertConstructionIssue);
  ...
  const allReplacementProducts = data?.products ?? [];
  const replacementProducts = allReplacementProducts.filter(
    (product: any) => !replacementForm.category || (product.category?.trim() || "ללא קטגוריה") === replacementForm.category,
  );
  const replacementProductSource = replacementProductSearch.trim() ? allReplacementProducts : replacementProducts;
  const replacementProductResults = replacementProductSource.filter((product: any) => {
    const q = replacementProductSearch.trim().toLowerCase();
    if (!q) return true;
    return `${product.name} ${product.id} ${product.category ?? ""} ${product.description ?? ""}`.toLowerCase().includes(q);
  }).slice(0, 30);
  ...
  const visibleIssues = useMemo(() => {
    ...
    return (data?.construction_issues ?? []).filter((issue: any) => {
      if (issueStatusFilter !== "all" && issue.status !== issueStatusFilter) return false;
      if (q && !`${issue.location_label} ${issue.description} ${issue.notes ?? ""}`.toLowerCase().includes(q)) return false;
      return true;
    });
  }, [data?.construction_issues, issueSearch, issueStatusFilter]);
  ...
  function openEditIssue(issue: any) {
  ...
  async function saveIssue() {
    ...
    try {
      await upsertIssueFn({
        data: {
          id: issueEditing?.id,
          location_type: issueForm.location_type as any,
          location_label: issueForm.location_label,
          description: issueForm.description,
          opened_at: issueForm.opened_at,
      ...
      await qc.invalidateQueries({ queryKey: ["rasp-dashboard"] });
      toast.success("תקלת הבינוי נשמרה");
    } catch (e: any) {
      toast.error(e.message || "שגיאה בשמירת התקלה");
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="min-h-screen bg-secondary/25" dir="rtl">
      ...
      <main className="max-w-7xl mx-auto p-4 space-y-5">
        ...
        <section>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
            <div>
              <h2 className="text-xl font-bold">תקלות בינוי</h2>
              <p className="text-sm text-muted-foreground">ניהול תקלות במכולות, חדרים ומיקומים נוספים ביחידה</p>
            </div>
            <Button onClick={openNewIssue}><PackagePlus className="w-4 h-4 ml-1" />פתיחת תקלה</Button>
          </div>
          <Card className="p-3 mb-3 flex flex-col md:flex-row gap-2">
            <div className="relative flex-1">
              <Search className="w-4 h-4 absolute right-3 top-3 text-muted-foreground" />
              <Input className="pr-9" placeholder="חיפוש לפי מיקום או תיאור" value={issueSearch} onChange={(e) => setIssueSearch(e.target.value)} />
            </div>
            <Select value={issueStatusFilter} onValueChange={setIssueStatusFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="resolved">טופלה</SelectItem>
              </SelectContent>
            </Select>
            {(issueSearch || issueStatusFilter !== "all") && (
              <Button variant="outline" onClick={() => { setIssueSearch(""); setIssueStatusFilter("all"); }}>נקה סינון</Button>
            )}
          </Card>
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-3">
            {!visibleIssues.length && <Card className="p-8 text-center text-muted-foreground md:col-span-2 xl:col-span-3">לא נמצאו תקלות בינוי</Card>}
            {visibleIssues.map((issue: any) => (
              <Card key={issue.id} className={`p-4 space-y-3 ${issue.status === "resolved" ? "opacity-75" : "border-amber-500/30"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="font-semibold">{locationTypeLabels[issue.location_type] ?? issue.location_type} · {issue.location_label}</h3>
                    <p className="text-xs text-muted-foreground">{dateFormat.format(new Date(`${issue.opened_at}T00:00:00`))}</p>
                  </div>
                  <Badge variant="secondary" className={issue.status === "resolved" ? "bg-emerald-500/10 text-emerald-700" : issue.status === "in_progress" ? "bg-amber-500/10 text-amber-700" : ""}>
                    {constructionStatusLabels[issue.status] ?? issue.status}
                  </Badge>
                </div>
                <p className="text-sm whitespace-pre-wrap">{issue.description}</p>
                {issue.notes && <div className="text-xs rounded bg-muted/60 p-2 whitespace-pre-wrap">{issue.notes}</div>}
                <div className="flex gap-2">
                  <Button variant="outline" className="flex-1" onClick={() => openEditIssue(issue)}>צפייה / עריכה</Button>
                  {issue.status !== "resolved" && (
                    <Button variant="outline" onClick={() => {
                      ...
                      setIssueForm({
                        ...
                        description: issue.description,
                        opened_at: issue.opened_at,
                        status: issue.status === "new" ? "in_progress" : "resolved",
                        notes: issue.notes ?? "",
                      });
                    }}>{issue.status === "new" ? "סמן בטיפול" : "סמן טופלה"}</Button>
                  )}
                </div>
              </Card>
            ))}
        ...
        <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <label className="text-sm space-y-1 block">
              ...
              <Input
                className="mb-2"
                placeholder="חיפוש פריט לפי שם, מזהה או קטגוריה"
                value={replacementProductSearch}
                onChange={(e) => {
                  setReplacementProductSearch(e.target.value);
                  setReplacementForm((form) => ({ ...form, product_id: "" }));
                }}
              />
              <Select value={replacementForm.product_id} onValueChange={(value) => {
                const product = (data.products ?? []).find((item: any) => item.id === value);
                setReplacementForm((form) => ({ ...form, product_id: value, category: product?.category?.trim() || form.category }));
              }}>
                <SelectTrigger><SelectValue placeholder={replacementForm.category ? "בחר פריט החלפה" : "בחר קטגוריה תחילה"} /></SelectTrigger>
                <SelectContent>{replacementProductResults.map((product: any) => <SelectItem key={product.id} value={product.id}>{product.name}</SelectItem>)}</SelectContent>
              </Select>
            </label>
      ...
      <Dialog open={!!issueEditing} onOpenChange={(open) => !open && setIssueEditing(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{issueEditing?.id ? "עריכת תקלת בינוי" : "פתיחת תקלת בינוי"}</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <label className="text-sm space-y-1 block">
              <span>מיקום התקלה</span>
              <Select value={issueForm.location_type} onValueChange={(value) => setIssueForm({ ...issueForm, location_type: value })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="container">מכולה</SelectItem>
                  <SelectItem value="room">חדר</SelectItem>
                  <SelectItem value="other">אחר</SelectItem>
                </SelectContent>
              </Select>
            </label>
            <Field label="מספר או שם המכולה / החדר">
              <Input value={issueForm.location_label} onChange={(e) => setIssueForm({ ...issueForm, location_label: e.target.value })} />
            </Field>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <Field label="תאריך פתיחה">
                <Input type="date" dir="ltr" value={issueForm.opened_at} onChange={(e) => setIssueForm({ ...issueForm, opened_at: e.target.value })} />
              </Field>
              <label className="text-sm space-y-1 block">
                <span>סטטוס</span>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIssueEditing(null)}>ביטול</Button>
            <Button disabled={saving || !issueForm.location_label || issueForm.description.trim().length < 3} onClick={saveIssue}>
              {saving && <Loader2 className="w-4 h-4 animate-spin ml-1" />}שמירה
            </Button>
          </DialogFooter>
        </DialogContent>
